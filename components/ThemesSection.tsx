      ...
      <div className="grid grid-flow-row lg:grid-cols-5 gap-5 grid-cols-1 my-10">
        {ThemeItems.map((item, index) => {
          return (
            <div
              key={index}
              className="grid group flip-card grid-flow-col cursor-pointer grid-rows-3 items-center h-[450px] px-10 border bg-gradient-to-b from-black via-[#3B193D] to-[#3B193D]  py-10 gap-4 text-center"
            >
              <h5 className="text-xl text-white font-bold ">{item.title}</h5>
              <div className="text-[#ED00B8] group-hover:hidden underline ">
                Know More
              </div>
              <h6 className="text-sm  text-white group-hover:block hidden  font-[400]">
                {item.description}
              </h6>
              <Image
                className="m-auto mb-5 group-hover:hidden"
                src={item.logo}
                alt=""
                width={80}
                height={80}
